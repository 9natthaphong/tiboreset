...
The homepage, cinematic hero, forecast views, Latest Signals, `/api/hybrid/current`, the read-only inspector, and Data Lab all consume this canonical snapshot. A completed full or banked reset, or a bounded official scheduled-reset announcement, is deterministically validated before it can become the newest announcement-cycle boundary. The announcement resolves the previous forecast at publication time; a scheduled event remains labeled scheduled and is not misrepresented as completed execution. The next cycle starts at zero maturity, transient evidence at or before the boundary is excluded, the resolving post contributes zero to the active signal channel, and Reset Oracle v2 is recalculated from cutoff-safe evidence. A still-current official `reset_policy_continuation` regime is the sole intentional cross-cycle exception because it describes future policy rather than one resolved event.
...
Sacred Watch 2.1 uses max-channel evidence fusion. The channels are Reset Oracle v2 timing, independent elapsed-cycle pressure, policy confidence moderated by normalized cycle maturity and policy age, and the strongest eligible structured live signal. The cycle-pressure channel reuses the cutoff-safe monotonic cycle estimator and verified inter-reset cadence. Because the interval sample remains sparse, the current channel is explicitly an expert-prior operational transform rather than an empirical probability. Related channels and posts do not sum. The maximum channel wins, then the strongest bounded negative penalty is applied once.
