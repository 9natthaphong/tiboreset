...
For the current public snapshot, a stored extraction is not trusted as a forecast-resolving reset solely because of its label. Completed full or banked resets require the monitored official account, no review flag, confirmation-safe confidence, and deterministic completed operational language. A scheduled announcement separately requires explicit operational reset wording plus a bounded near-term publication schedule; it resolves the announcement target without being presented as completed execution. Derived resolution state records source, extraction version, verification method, resolution kind, and synchronization into `known_reset_events` or `milestone_events`.
