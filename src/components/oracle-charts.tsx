          ...
          <section id="forecast-panel-movement" role="tabpanel" aria-labelledby="forecast-tab-movement" className="forecast-view-panel movement-view" data-testid="probability-trend">
            <header className="forecast-view-heading">
              <div>
                ...
                {trendView === "live" && <p>This chart tracks Reset Oracle v2 probability, not the Reset Watch Score.</p>}
              </div>
            ...
            {trendView === "live" ? (
              <>
                {hybrid?.eventResolutionStatus === "resolved" && hybrid.confirmation && <div className="forecast-cycle-resolution" role="status">
                  <div><span>{hybrid.confirmation.resetType === "scheduled" ? "RESET ANNOUNCED" : "RESET RELEASED"}</span><strong>{formatUtcTimestamp(hybrid.confirmation.occurredAt)}</strong><p>The previous forecast resolved at {hybrid.previousCycleFinalProbability == null ? "the official announcement" : `${Math.round(hybrid.previousCycleFinalProbability * 100)}%`}. The active forecast starts a new visual cycle.</p></div>
      ...
      <details className="advanced-diagnostics" open={diagnosticsOpen} onToggle={(event) => setDiagnosticsOpen(event.currentTarget.open)} data-testid="advanced-diagnostics">
        ...
        {diagnosticsOpen && (
          <div className="diagnostics-disclosure">
            {hybrid && <section className="hybrid-model-record" aria-labelledby="hybrid-model-title">
              <header><span>RESET WATCH SCORE</span><h3 id="hybrid-model-title">{hybrid.watchScore} / 100</h3><p>An operational readiness score, not a probability. {hybrid.whyThisScore}</p></header>
              {hybrid.eventResolutionStatus === "resolved" && hybrid.confirmation && <div className="resolved-event-audit"><span>RESOLVED EVENT</span><strong>{hybrid.confirmation.resetType === "scheduled" ? "Scheduled reset announced" : hybrid.confirmation.resetType === "banked" ? "Banked reset released" : "Full reset released"}</strong><p>{formatUtcTimestamp(hybrid.confirmation.occurredAt)} · previous forecast {hybrid.previousCycleFinalProbability == null ? "record unavailable" : `${Math.round(hybrid.previousCycleFinalProbability * 100)}%`}</p>{hybrid.confirmation.sourceUrl && <a href={hybrid.confirmation.sourceUrl} target="_blank" rel="noreferrer">Official source <ExternalLink size={13}/></a>}</div>}
              <dl><div><dt>Watch model</dt><dd>{hybrid.watchModelVersion}</dd></div><div><dt>Winning channel</dt><dd>{hybrid.maxWinningChannel.replaceAll("_", " ")}</dd></div><div><dt>Calibrated timing channel</dt><dd>{hybrid.timingChannel.toFixed(3)}</dd></div><div><dt>Cycle-pressure channel</dt><dd>{hybrid.cyclePressureChannel.toFixed(3)}</dd></div><div><dt>Policy-timing channel</dt><dd>{hybrid.policyTimingChannel.toFixed(3)}</dd></div><div><dt>Strongest signal channel</dt><dd>{hybrid.strongestSignalChannel.toFixed(3)}</dd></div><div><dt>Negative penalty</dt><dd>{hybrid.negativePenalty.toFixed(3)}</dd></div><div><dt>Cycle maturity</dt><dd>{hybrid.cycleMaturity.toFixed(3)}</dd></div><div><dt>Elapsed / expected cycle</dt><dd>{hybrid.elapsedCycleHours.toFixed(1)}h / {hybrid.expectedCycleHours.toFixed(1)}h</dd></div><div><dt>Cycle method</dt><dd>{hybrid.cyclePressureMethod.replaceAll("_", " ")}</dd></div><div><dt>Calibrated next-36h probability</dt><dd>{Math.round(hybrid.calibratedProbability * 100)}%</dd></div><div><dt>Override</dt><dd>{hybrid.appliedOverride ?? "None"}</dd></div></dl>
              <p className="model-record-note">Cycle pressure rises as the active interval ages, but it is an operational readiness signal, not a calibrated probability.</p>
            </section>}
            {hybrid && <section className="policy-regime-record" aria-labelledby="policy-regime-title"><header><span>RESET POLICY</span><h3 id="policy-regime-title">{hybrid.policyRegimeState.replaceAll("_", " ")}</h3><p>{hybrid.policyRegimeReason}</p></header><dl><div><dt>Source post</dt><dd>{hybrid.policyRegimeSourcePostId ?? "None"}</dd></div><div><dt>Activated</dt><dd>{hybrid.policyRegimeActivatedAt ? formatUtcTimestamp(hybrid.policyRegimeActivatedAt) : "Inactive"}</dd></div><div><dt>Expires</dt><dd>{hybrid.policyRegimeExpiresAt ? formatUtcTimestamp(hybrid.policyRegimeExpiresAt) : "Inactive"}</dd></div><div><dt>Evidence confidence</dt><dd>{Math.round(hybrid.policyRegimeConfidence * 100)}%</dd></div><div><dt>Age</dt><dd>{hybrid.policyRegimeAgeHours == null ? "Inactive" : `${hybrid.policyRegimeAgeHours.toFixed(1)} hours`}</dd></div><div><dt>Decay</dt><dd>{hybrid.policyRegimeDecayFactor.toFixed(3)}</dd></div><div><dt>Policy-timing channel</dt><dd>{hybrid.policyTimingChannel.toFixed(3)}</dd></div><div><dt>Watch counterfactual</dt><dd>{hybrid.policyRegimeWatchCounterfactualDeltaPoints == null ? "Unavailable" : `${hybrid.policyRegimeWatchCounterfactualDeltaPoints > 0 ? "+" : ""}${hybrid.policyRegimeWatchCounterfactualDeltaPoints} points`}</dd></div><div><dt>Calibrated counterfactual</dt><dd>{hybrid.policyRegimeCalibratedCounterfactualDeltaPercentagePoints == null ? "Unavailable" : `${hybrid.policyRegimeCalibratedCounterfactualDeltaPercentagePoints.toFixed(1)} points`}</dd></div></dl><p className="model-record-note">Policy confidence is distinct from timing readiness. The seven-day lifetime and 72-hour full-strength window are transparent expert product priors.</p></section>}
