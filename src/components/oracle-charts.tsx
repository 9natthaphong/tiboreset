      ...
      <details className="advanced-diagnostics" open={diagnosticsOpen} onToggle={(event) => setDiagnosticsOpen(event.currentTarget.open)} data-testid="advanced-diagnostics">
        ...
        {diagnosticsOpen && (
          <div className="diagnostics-disclosure">
            {hybrid && <section className="hybrid-model-record" aria-labelledby="hybrid-model-title"><header><span>RESET WATCH SCORE</span><h3 id="hybrid-model-title">{hybrid.watchScore} / 100</h3><p>An operational readiness score, not a probability. {hybrid.whyThisScore}</p></header>{hybrid.eventResolutionStatus === "resolved" && hybrid.confirmation && <div className="resolved-event-audit"><span>RESOLVED EVENT</span><strong>{hybrid.confirmation.resetType === "banked" ? "Banked reset released" : "Full reset released"}</strong><p>{formatUtcTimestamp(hybrid.confirmation.occurredAt)} · previous forecast {hybrid.previousCycleFinalProbability == null ? "record unavailable" : `${Math.round(hybrid.previousCycleFinalProbability * 100)}%`}</p>{hybrid.confirmation.sourceUrl && <a href={hybrid.confirmation.sourceUrl} target="_blank" rel="noreferrer">Official source <ExternalLink size={13}/></a>}</div>}<dl><div><dt>Watch model</dt><dd>{hybrid.watchModelVersion}</dd></div><div><dt>Winning channel</dt><dd>{hybrid.maxWinningChannel.replaceAll("_", " ")}</dd></div><div><dt>Timing channel</dt><dd>{hybrid.timingChannel.toFixed(3)}</dd></div><div><dt>Policy-timing channel</dt><dd>{hybrid.policyTimingChannel.toFixed(3)}</dd></div><div><dt>Strongest signal channel</dt><dd>{hybrid.strongestSignalChannel.toFixed(3)}</dd></div><div><dt>Negative penalty</dt><dd>{hybrid.negativePenalty.toFixed(3)}</dd></div><div><dt>Cycle maturity</dt><dd>{hybrid.cycleMaturity.toFixed(3)}</dd></div><div><dt>Elapsed / expected cycle</dt><dd>{hybrid.elapsedCycleHours.toFixed(1)}h / {hybrid.expectedCycleHours.toFixed(1)}h</dd></div><div><dt>Calibrated next-36h probability</dt><dd>{Math.round(hybrid.calibratedProbability * 100)}%</dd></div><div><dt>Override</dt><dd>{hybrid.appliedOverride ?? "None"}</dd></div></dl></section>}
