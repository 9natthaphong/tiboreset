...
export default function AdvancedDiagnostics({ forecast }: { forecast: Forecast }) {
  ...
  return <div className="advanced-diagnostics-content">
    ...
    <div className="coefficient-table" role="table" aria-label="Forecast coefficients"><div role="row"><b>Display label</b><b>Technical feature</b><b>Value</b><b>Coefficient</b><b>Contribution</b></div>{forecast.contributions.map(item => <div role="row" key={item.featureName}><span>{item.label}</span><code>{item.featureName}</code><span>{item.normalizedValue.toFixed(2)}</span><span>{item.coefficient.toFixed(2)}</span><span>{item.logOddsContribution.toFixed(3)}</span></div>)}</div>
