  ...
  return <main ref={mainRef} data-refresh-ready="false">
    ...
    <section id="latest-signals" className="section latest-signals-section" data-editorial-section>
      ...
      <div className="ingestion-forecast-status" aria-label="Latest ingestion and forecast status">
        <span><small>Last X check</small><b>{health.lastIngestionAt ? formatTimestamp(health.lastIngestionAt) : "Not yet checked"}</b></span>
...
function LatestPostCard({ post, mode, expanded, onToggle, now }: { post: LatestPost; mode: "demo" | "live"; expanded: boolean; onToggle: () => void; now: string }) {
  ...
  const result = resolvedReset ? "Completed confirmation" : activePolicy ? "Ongoing policy" : contribution > 0 ? "Raised" : contribution < 0 ? "Lowered" : "Contextual";
  return <article className={`signal-card impact-${contribution > 0 ? "positive" : contribution < 0 ? "negative" : "neutral"}`} data-testid="latest-post-card">
    ...
    {activePolicy && <div className="signal-policy-meta"><span>Policy active</span><span>{Math.round((post.policyRegimeDecayFactor ?? 0) * 100)}% regime strength</span><span>Policy-only cap {post.policyRegimeCap ?? 80}%</span>{post.probabilityCounterfactualDeltaPercentagePoints != null && <span>Calibrated counterfactual {post.probabilityCounterfactualDeltaPercentagePoints.toFixed(1)} pts</span>}</div>}
