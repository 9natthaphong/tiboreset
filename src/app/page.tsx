...
export default async function Page() {
  ...
  return <OracleExperience initialForecast={snapshot.forecast} initialHybrid={snapshot.hybrid} hybridStatus={snapshot.hybridStatus} evidence={snapshot.evidence} history={snapshot.history} latestPosts={snapshot.latestPosts} resetHistory={snapshot.resetHistory} milestoneState={snapshot.milestoneState} historicalDataset={snapshot.historicalDataset} externalContextEvents={snapshot.externalContextEvents} health={snapshot.health} analogs={analogs} renderedAt={new Date().toISOString()} evidenceExtractionModel={evidenceExtractionModel} backtestSummary={backtestSummary}/>;
}
