...
export default async function DataLabPage() {
  ...
  const latestMilestone = [...data.seed.resetLedger].sort((left, right) => right.milestoneUsers - left.milestoneUsers)[0];
  ...
  return <main className="lab data-lab">
    ...
    <section aria-labelledby="combined-milestone"><p className="lab-section-index">11</p><h2 id="combined-milestone">CURRENT COMBINED MILESTONE</h2>{latestMilestone ? <div className="milestone-data-record"><strong>{(latestMilestone.milestoneUsers / 1_000_000).toFixed(0)}M</strong><div><b>Codex + ChatGPT Work combined active users</b><p>The July 9M figure is a combined denominator. The latest official Codex-only figure was 5M+ weekly users on June 2.</p><a href={latestMilestone.sourceUrl} target="_blank" rel="noreferrer">Official announcement ↗</a></div></div> : <p>No reviewed milestone is available.</p>}</section>

    <section aria-labelledby="next-pledged-milestone"><p className="lab-section-index">12</p><h2 id="next-pledged-milestone">NEXT PLEDGED MILESTONE</h2><div className="milestone-data-record"><strong>{data.nextPledgedMilestoneUsers / 1_000_000}M</strong><div><b>90% milestone progress</b><p>The 10M target is pledged, not reached or confirmed. The feature uses 9M divided by 10M only when the 9M source is available before the forecast cutoff.</p></div></div></section>
