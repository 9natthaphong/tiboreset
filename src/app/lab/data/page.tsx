...
export default async function DataLabPage() {
  ...
  return <main className="lab data-lab">
    ...
    <section aria-labelledby="hybrid-record"><p className="lab-section-index">03A</p><h2 id="hybrid-record">CANONICAL HYBRID SNAPSHOT</h2>{canonical ? <><div className="lab-banner" role="status">{canonical.hybrid.hybridState.replaceAll("_", " ").toUpperCase()} · SCORE {canonical.hybrid.hybridScore} · CALIBRATED {Math.round(canonical.forecast.probability * 100)}%</div><p>The confirmed reset closes the previous forecast. Current scores estimate a future reset after that event.</p>
      ...
      <article className="data-lab-cycle-record active"><span>ACTIVE NEXT-RESET FORECAST</span><h3>{canonical.hybrid.hybridScore}% Live Reset Likelihood</h3><dl className="canonical-hybrid-record"><div><dt>Hybrid model</dt><dd>{canonical.hybrid.hybridModelVersion}</dd></div><div><dt>activeCycleStartAt</dt><dd>{canonical.hybrid.cycleStartAt ?? "Unavailable"}</dd></div><div><dt>activeCycleHybridScore</dt><dd>{canonical.hybrid.hybridScore}</dd></div><div><dt>activeCycleCalibratedProbability</dt><dd>{canonical.forecast.probability}</dd></div><div><dt>Elapsed cycle</dt><dd>{canonical.hybrid.elapsedCycleHours.toFixed(2)} hours</dd></div><div><dt>Expected cycle</dt><dd>{canonical.hybrid.expectedCycleHours.toFixed(2)} hours</dd></div><div><dt>Cycle points</dt><dd>{canonical.hybrid.cyclePoints.toFixed(3)}</dd></div><div><dt>Historical points</dt><dd>{canonical.hybrid.historicalPoints.toFixed(3)}</dd></div><div><dt>Signal points</dt><dd>{canonical.hybrid.signalPoints.toFixed(3)}</dd></div><div><dt>Negative points</dt><dd>{canonical.hybrid.negativePoints.toFixed(3)}</dd></div><div><dt>Override</dt><dd>{canonical.hybrid.appliedOverride ?? "None"}</dd></div><div><dt>Canonical cutoff</dt><dd>{canonical.cutoff}</dd></div><div><dt>preCycleSignalExclusions</dt><dd>{canonical.hybrid.excludedSignals.filter(item => item.exclusionReason === "before_cycle_start").length}</dd></div></dl></article>
      <article className="data-lab-cycle-record active"><span>LONG-LIVED RESET POLICY</span><h3>{canonical.hybrid.policyRegimeState.replaceAll("_", " ")}</h3><dl className="canonical-hybrid-record"><div><dt>policyRegimeSourcePostId</dt><dd>{canonical.hybrid.policyRegimeSourcePostId ?? "None"}</dd></div><div><dt>policyRegimeActivatedAt</dt><dd>{canonical.hybrid.policyRegimeActivatedAt ?? "None"}</dd></div><div><dt>policyRegimeExpiresAt</dt><dd>{canonical.hybrid.policyRegimeExpiresAt ?? "None"}</dd></div><div><dt>policyRegimeConfidence</dt><dd>{canonical.hybrid.policyRegimeConfidence}</dd></div><div><dt>policyRegimeReason</dt><dd>{canonical.hybrid.policyRegimeReason}</dd></div><div><dt>policyContinuationBoost</dt><dd>{canonical.hybrid.policyContinuationBoost}</dd></div><div><dt>policyRegimeScoreFloor</dt><dd>{canonical.hybrid.policyRegimeScoreFloor}</dd></div><div><dt>policyRegimeCap</dt><dd>{canonical.hybrid.policyRegimeCap}</dd></div><div><dt>policyRegimeDecayFactor</dt><dd>{canonical.hybrid.policyRegimeDecayFactor}</dd></div><div><dt>calibratedCounterfactualDeltaPercentagePoints</dt><dd>{canonical.hybrid.policyRegimeCalibratedCounterfactualDeltaPercentagePoints ?? "Unavailable"}</dd></div></dl><p>The 72-hour full-strength window and seven-day expiry are transparent expert product priors, not statistically learned parameters.</p></article>
      <details><summary>Structured signal contributions and exclusions</summary><pre>{JSON.stringify({ active: canonical.hybrid.activeSignals, excluded: canonical.hybrid.excludedSignals, policyRegime: { state: canonical.hybrid.policyRegimeState, sourcePostId: canonical.hybrid.policyRegimeSourcePostId, activatedAt: canonical.hybrid.policyRegimeActivatedAt, expiresAt: canonical.hybrid.policyRegimeExpiresAt, confidence: canonical.hybrid.policyRegimeConfidence, reason: canonical.hybrid.policyRegimeReason, boost: canonical.hybrid.policyContinuationBoost, floor: canonical.hybrid.policyRegimeScoreFloor, cap: canonical.hybrid.policyRegimeCap, decayFactor: canonical.hybrid.policyRegimeDecayFactor, calibratedCounterfactualDeltaPercentagePoints: canonical.hybrid.policyRegimeCalibratedCounterfactualDeltaPercentagePoints }, resolvedEvent: canonical.hybrid.confirmation, persistedForecast: canonical.persistedForecast, resolvedForecast: canonical.resolvedForecast }, null, 2)}</pre></details></> : <p>Canonical hybrid state is unavailable. No operational score is fabricated.</p>}</section>
