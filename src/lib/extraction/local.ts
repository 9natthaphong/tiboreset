...
import { classifyResetPolicyLanguage, enforceExtractionSafety, hasCredibleOperationalResetCommitment, hasExplicitCompletedOperationalReset } from "./safety";
...
export function localExtract(text: string): Extraction {
  ...
  const extraction: Extraction = {
    ...
    commitment_strength: explicitReset ? .95 : policyContinuation ? .85 : milestone?.resetType === "scheduled" ? .85 : credibleCommitment ? .85 : resetHint ? .55 : 0,
    ...
    time_reference: milestone?.resetType === "scheduled" || /tomorrow|soon|next/.test(lower) ? "near_future" : policyContinuation ? "future_unspecified" : relevant ? "current" : "none",
    reset_type: explicitReset ? milestone?.resetType ?? "unknown" : milestone?.resetType ?? (resetHint ? "unknown" : "none"),
    ...
    extraction_confidence: explicitReset && !ambiguous ? .96 : policyContinuation ? .92 : milestone && !ambiguous ? .9 : relevant ? .65 : .9,
    ...
    reset_intent_strength: explicitReset ? 1 : policyContinuation ? .9 : policyLanguage === "withdrawal" ? 1 : credibleCommitment ? .85 : resetHint ? .55 : operatorIntervention ? .15 : 0,
