...
async function main() {
  ...
  const output: Record<string, unknown> = {
    ...
    components: { cyclePoints: snapshot.hybrid.cyclePoints, historicalPoints: snapshot.hybrid.historicalPoints, signalPoints: snapshot.hybrid.signalPoints, negativePoints: snapshot.hybrid.negativePoints, policyContinuationBoost: snapshot.hybrid.policyContinuationBoost, policyRegimeEffectivePoints: snapshot.hybrid.policyRegimeEffectivePoints, appliedOverride: snapshot.hybrid.appliedOverride },
    policyRegime: { state: snapshot.hybrid.policyRegimeState, sourcePostId: snapshot.hybrid.policyRegimeSourcePostId, activatedAt: snapshot.hybrid.policyRegimeActivatedAt, expiresAt: snapshot.hybrid.policyRegimeExpiresAt, confidence: snapshot.hybrid.policyRegimeConfidence, reason: snapshot.hybrid.policyRegimeReason, floor: snapshot.hybrid.policyRegimeScoreFloor, cap: snapshot.hybrid.policyRegimeCap, ageHours: snapshot.hybrid.policyRegimeAgeHours, decayFactor: snapshot.hybrid.policyRegimeDecayFactor, calibratedCounterfactualDeltaPercentagePoints: snapshot.hybrid.policyRegimeCalibratedCounterfactualDeltaPercentagePoints },
